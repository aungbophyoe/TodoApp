...
class TodoMapper @Inject constructor() : EntityMapper<TodoEntity, Todo> {
    override fun mapFromEntity(entity: TodoEntity): Todo {
        return Todo(
            id = entity.id, title = entity.title,isCompleted = entity.isCompleted, date = entity.date
        )
    }

    override fun mapToEntity(domainModel: Todo): TodoEntity {
        return TodoEntity(
            id = domainModel.id, title = domainModel.title,isCompleted = domainModel.isCompleted, date = domainModel.date
        )
    }
}